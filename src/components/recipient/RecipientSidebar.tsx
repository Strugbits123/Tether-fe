...
export default function RecipientSidebar({
  ...
  // My Profile opens the shared profile modal; Overview is the live page. The
  // remaining content sections are static placeholders for now.
  const PRIMARY_NAV: NavItem[] = [
    { label: 'My Profile', icon: User, onClick: () => setProfileOpen(true) },
    { label: 'Overview', icon: Home, href: '/recipient' },
    { label: 'Messages', icon: MessageSquare, href: '/recipient/messages' },
    { label: 'Docs & Files', icon: FileText, href: '/recipient/docs' },
    { label: 'Memories', icon: ImageIcon, href: '/recipient/memories' },
    { label: 'Memoir', icon: BookOpen, href: '/recipient/memoir' },
    { label: 'Notifications', icon: Bell, href: '/recipient/notifications' },
  ]
      ...
      >
        ...
        >
          ...
          <nav className="flex flex-col" style={{ gap: 4 }}>
            ...
            {/* Create my Tether — promotes the main app */}
            <button
              type="button"
              onClick={() => {
                router.push('/recipient/create-account')
                onClose()
              }}
